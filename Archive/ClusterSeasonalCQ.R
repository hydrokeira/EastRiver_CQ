...
solute_clusters_CC<-month_clusters[,c(1,2)]

write.csv(solute_clusters_CC, "CC_CQ_Clusters.csv")

dl<-read.csv("/Users/keirajohnson/Box Sync/Keira_Johnson/ER_CQ/PH_Cation_DL.csv")

colnames(dl)[3]<-"variable_units"

dl$Solute<-sub("_.*", "", dl$variable_units)

dl[45,5]<-"ammonia_n"

month_clusters<-full_join(month_clusters, dl, by="Solute")

month_clusters<-month_clusters[complete.cases(month_clusters),]

month_clusters_melt<-melt(month_clusters[,c(1:14,18)], id.vars = c("Solute", "Cluster","Class"))

#plot lines by cluster
p2<-ggplot(month_clusters_melt, aes(variable, value))+
  geom_line(aes(col=Class, group=Solute), linewidth=0.7)+
...
p4<-ggplot(month_clusters_melt, aes(variable, value))+
  geom_line(aes(col=Class, group=Solute), size=0.7, alpha=0.45)+
  geom_smooth(aes(group=Cluster), se=F, col="black")+
  theme_bw()+facet_wrap(~Cluster, nrow = 1)+
  scale_color_manual(values = c("biologic"="dodgerblue", "metal"="goldenrod", "geogenic"="firebrick"))+
  theme(text = element_text(size = 20), legend.position = "null")+
  labs(x="Month", y="Normalized CQ Slope")+
  ggtitle("Coal Creek")

p4

p6<-ggplot(month_clusters_melt, aes(x=Cluster, fill=Class))+
  ...
  labs(x="Cluster", y="Proportion", fill="")

p6

p8<-ggarrange(p4, p6, widths = c(0.6, 0.4))
...
colnames(month_clusters)<-c("Cluster", paste(seq(1:12)))
month_clusters<-rownames_to_column(month_clusters, "Solute")
month_clusters$Cluster_reordered<-ifelse(month_clusters$Cluster==1, 2,
                                         ifelse(month_clusters$Cluster==2, 1, 3))
#month_clusters$Site<-str_sub(month_clusters$SiteYear,1,nchar(month_clusters$SiteYear)-5)
...
solute_clusters_ER<-month_clusters[,c(1,15)]

write.csv(solute_clusters_ER, "ER_CQ_Clusters.csv")

month_clusters_melt<-melt(month_clusters[,c(1:15,19)], id.vars = c("Solute", "Cluster","Cluster_reordered","Class"))
...
p2<-ggplot(month_clusters_melt, aes(variable, value))+
  ...
  theme(text = element_text(size = 20), legend.position = "bottom")+
  labs(col="Solute Class", x="Month", y="Normalized CQ Slope")+
  ggtitle("East River Creek")

p2

p3<-ggplot(month_clusters_melt, aes(variable, value))+
  geom_line(aes(col=Class, group=Solute), size=0.7, alpha=0.45)+
  geom_smooth(aes(group=Cluster_reordered), se=F, col="black")+
  theme_bw()+facet_wrap(~Cluster_reordered, nrow = 1)+
  scale_color_manual(values = c("biologic"="dodgerblue", "metal"="goldenrod", "geogenic"="firebrick"))+
  theme(text = element_text(size = 20), legend.position = "null")+
  labs(x="Month", y="Normalized CQ Slope")+
  ggtitle("East River")
...
p5<-ggplot(month_clusters_melt, aes(x=Cluster_reordered, fill=Class))+
  geom_bar(stat = "count", position = "fill")+
  #geom_line(aes(col=class, group=Solute), size=0.7)+
  theme_bw()+
  scale_fill_manual(values = c("biologic"="dodgerblue", "metal"="goldenrod", "geogenic"="firebrick"))+
...
p7<-ggarrange(p3, p5, widths = c(0.6, 0.4))

p7

pdf("Normalized_Clusters.pdf", width = 15, height = 8)

ggarrange(p7, p8, nrow=2, heights = c(0.47, 0.53))

dev.off()


colnames(ercq_monthly)[2]<-"Solute"
ercq_monthly<-left_join(ercq_monthly, solute_clusters_ER, by="Solute")
ercq_monthly<-ercq_monthly[complete.cases(ercq_monthly),]
...
ercq_monthly<-left_join(ercq_monthly, solute_class, by="Solute")
ercq_monthly<-left_join(ercq_monthly, conversion, by="number")

p2<-ggplot(ercq_monthly, aes(month, slope))+
  geom_line(aes(col=class, group=Solute), size=0.7)+
  theme_bw()+facet_wrap(~Cluster_reordered, nrow = 1)+
...
p4<-ggplot(ercq_monthly, aes(month, slope))+
  geom_boxplot(aes(group=month), outliers = F)+
  geom_jitter(aes(col=class), alpha=0.7)+
  theme_bw()+facet_wrap(~Cluster_reordered, nrow = 1)+
  scale_color_manual(values = c("bioavailable"="dodgerblue", "metal"="goldenrod", "geogenic"="firebrick"))+
  theme(text = element_text(size = 20), legend.position = "null")+
  labs(x="Month", y="CQ Slope")+
  ylim(-2,2)

p6<-ggarrange(p2, p4, nrow = 2)
p6

colnames(ercq_monthly)[2]<-"Solute"
ercq_monthly<-left_join(ercq_monthly, solute_clusters_CC, by="Solute")
ercq_monthly<-ercq_monthly[complete.cases(ercq_monthly),]
...
p3<-ggplot(ercq_monthly, aes(month, slope))+
  geom_line(aes(col=class, group=Solute), size=0.7)+
  theme_bw()+facet_wrap(~Cluster, nrow = 1)+
  scale_color_manual(values = c("bioavailable"="dodgerblue", "metal"="goldenrod", "geogenic"="firebrick"))+
  theme(text = element_text(size = 20), legend.position = "null")+
...
p5<-ggplot(ercq_monthly, aes(month, slope))+
  geom_boxplot(aes(group=month), outliers = F)+
  geom_jitter(aes(col=class), alpha=0.7)+
  theme_bw()+facet_wrap(~Cluster, nrow = 1)+
  scale_color_manual(values = c("bioavailable"="dodgerblue", "metal"="goldenrod", "geogenic"="firebrick"))+
...
p7<-ggarrange(p3, p5, nrow = 2)
p7

pdf("Raw_CQ_Clusters.pdf", width=18, height=8)
